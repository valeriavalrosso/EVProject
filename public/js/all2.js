...
// App methods
	...
	AJAXpopulateCalendar(){
	  ...
	  xhttp.onload = function() {
		...
		for(var i=0;i<parser.getElementsByClassName('date').length;i++){
			...
			hours[dateFormatted][hours[dateFormatted].indexOf(timeFormatted)]=hours[dateFormatted][hours[dateFormatted].indexOf(timeFormatted)]+"#selected#"+codFis
		...
		allDates.forEach((element, index) => { 
				...
				for (var l=0;l<hours[specificDate].length;l++){
					//alert(hours[dateFormatted]);		
						...
						if(hours[specificDate][l].includes("#selected")){
							
							hours[specificDate][l]=hours[specificDate][l].replace("#selected", "");
							var arr=hours[specificDate][l].split('#');
							calendarObj.eventList[specificDate].push('<button class="selectedHour">'+arr[0]+'</button><div class="cofFisOut">'+arr[1]+'</div>');
	...
	AJAXpopulateCalendar2(){
	  ...
	  xhttp.onload = function() {
		...
		allDates.forEach((element, index) => { 
				...
				var specificdate = specificDate.split('/');
				
				
				var specificdateFormatted = specificdate[2] + "/" + specificdate[1] + "/" + specificdate[0];
				
				var day = specificdate[0];
				
				var month = specificdate[1]-1;
				
				var year = specificdate[2];
				
				try{
					document.querySelectorAll('[data-day="'+day+'"][data-month="'+month+'"][data-year="'+year+'"]')[0].classList.add("event-day");
				}catch(v){}
	...
	addEventMarker(){
		daysToWork.forEach((element, index) => { 
				var specificDate=daysToWork[index];
